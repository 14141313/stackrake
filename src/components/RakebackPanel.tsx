...
export function RakebackPanel({ result, snapshots, tier }: Props) {
  ...
  const trueNetColor = trueNet > 0 ? 'text-positive' : trueNet < 0 ? 'text-negative' : 'text-gray-900'
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        ...
        <div>
          ...
          <div className="p-3 rounded-lg bg-gray-50 border border-gray-100 text-xs font-mono">
            ...
            <span className="text-gray-400 mx-2">·</span>
            <span className="text-gray-600">x{tierCfg.multiplier} GEMs</span>
          </div>
          <p className="text-xs text-gray-400 mt-1.5 italic">Update in account settings</p>
        ...
        <div>
          <label className="block text-xs text-gray-500 mb-2">
            GEM History
            {!hasSnapshots && <span className="text-gray-400 ml-2">— complete monthly check-ins to track</span>}
          </label>
              ...
              {monthlyGems.slice(-3).reverse().map(m => (
                <div key={m.month} className="flex justify-between items-center text-xs font-mono py-1.5 border-b border-gray-100 last:border-0">
                  ...
                  <div className="text-right">
                    ...
                    {m.redeemed > 0 && (
                      <span className="text-brand ml-2">{sign(m.cashValue)}</span>
                    )}
                  </div>
                </div>
              ))}
        ...
        <div className="space-y-3">
          ...
          <Row label={`Rakeback (${Math.round(tierPct * 100)}% tier)`} value={sign(rakeback)} valueColor="text-brand" />
            ...
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-6 gap-y-2 text-xs font-mono">
                ...
                <div className="col-span-full border-t border-gray-100 pt-2 mt-1 flex justify-between">
                  <span className="text-gray-700 font-semibold">Total permanent cost</span>
                  <span className="text-gray-900 font-semibold">${trueCost.toFixed(2)}</span>
                </div>
              </div>
            </div>
            ...
            {redistributed > 0 && (
              <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-6 gap-y-2 text-xs font-mono">
                  ...
                  <div className="col-span-full border-t border-gray-100 pt-2 mt-1 flex justify-between">
                    <span className="text-gray-700 font-semibold">Total redistributed</span>
                    <span className="text-gray-700 font-semibold">${redistributed.toFixed(2)}</span>
                  </div>
                </div>
              </div>
            )}
...
function DeductRow({ label, value, note, highlight }: { label: string; value: number; note?: string; highlight?: boolean }) {
  return (
    <div className="flex justify-between items-baseline">
      <span className={highlight ? 'text-orange-600' : 'text-gray-500'}>
        {label}{note ? <span className="text-gray-400 ml-1">({note})</span> : ''}
      </span>
      <span className={highlight ? 'text-orange-600' : 'text-gray-700'}>${value.toFixed(2)}</span>
    </div>
  )
}
